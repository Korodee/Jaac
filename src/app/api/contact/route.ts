...
export async function POST(req: Request) {
  try {
    ...
    if (!fullName || !email) {
      return new Response(JSON.stringify({ error: 'Nom complet et email requis.' }), { status: 400 });
    }
    ...
    if (!apiKey || !fromEmail || !toEmail) {
      ...
      return new Response(JSON.stringify({ error: 'Erreur de configuration du serveur email.' }), { status: 500 });
    }
